...
(() => {
  ...
  const TYPE_LABELS = {
    ...
    'book-chapter': 'Book chapter', thesis: 'Thesis', patent: 'Patent', report: 'Report',
  };
  const TYPE_ORDER = ['journal', 'conference', 'preprint', 'book-chapter'];
  const typeLabel = (t) => TYPE_LABELS[t] || (t ? t.charAt(0).toUpperCase() + t.slice(1).replace(/-/g, ' ') : 'Publication');
  ...
  function pubHTML(p, areasById, { compact = false } = {}) {
    ...
    const title = href ? `<a href="${esc(href)}"${ext}>${esc(p.title)}${hint}</a>` : esc(p.title);
    const areaTags = (p.areas || []).map((id) => areasById[id]).filter(Boolean)
      .map((a) => `<span class="pub__tag" style="--h:${Number(a.hue) || 210}">${esc(a.title)}</span>`).join('');
    const n = Number(p.citations);   // shown only when there is at least one; links to the Scopus record when known
    const cited = n > 0
      ? (p.scopus
        ? `<a class="pub__cited" href="${esc(p.scopus)}"${ext} title="Cited by ${n} on Scopus">Cited ${n}<span class="sr-only"> on Scopus (opens in a new tab)</span></a>`
        : `<span class="pub__cited">Cited ${n}</span>`)
      : '';
    ...
    if (!compact) {
      const buttons = [
        p.abstract ? `<button type="button" class="link-btn" data-action="abstract" aria-expanded="false" aria-controls="abs-${uid}">Abstract</button>` : '',
        p.doi ? `<a class="link-btn" href="https://doi.org/${esc(p.doi)}"${ext}>DOI${hint}</a>` : '',
        p.pdf ? `<a class="link-btn" href="${esc(p.pdf)}"${ext}>PDF${hint}</a>` : '',
        `<button type="button" class="link-btn" data-action="bibtex" data-id="${esc(p.id)}">Copy BibTeX</button>`,
      ].join('');
      actions = `<div class="pub__actions">${buttons}</div>`;
      if (p.abstract) {
        abstract = `<div class="pub__abstract" id="abs-${uid}" hidden><p>${esc(p.abstract)}</p>
          ${p.keywords && p.keywords.length ? `<p class="keywords">Keywords: ${p.keywords.map(esc).join(', ')}</p>` : ''}</div>`;
      }
    }
    const h = compact ? 'h3' : 'h4';
    return `<article class="pub">
      <${h} class="pub__title">${title}</${h}>
      <p class="pub__authors">${esc(authorList(p.authors))}</p>
      <p class="pub__venue">${venueHTML(p)}</p>
  ...
  async function initDashboard(app) {
    let data;
    try { data = await loadAll(); } catch (e) { app.innerHTML = ENE.errorNotice('the publications'); return; }
    ...
    if (updated && meta.updated && ENE.formatDate(meta.updated)) updated.textContent = `${meta.source ? `Data from ${meta.source}, last` : 'Data last'} updated ${ENE.formatDate(meta.updated)}`;
    ...
    el.readout.innerHTML = ENE.readoutHTML([
      ...
      { value: countBy((p) => p.type === 'journal'), label: 'Journal articles' },
      { value: countBy((p) => p.type === 'conference'), label: 'Conference papers' },
      cites ? { value: cites, label: 'Citations' } : { value: areas.length, label: 'Research themes' },
    ]);
    ...
    el.typeChips.insertAdjacentHTML('beforeend', chip('type', 'all', 'All', pubs.length)
      + types.map((t) => chip('type', t, typeLabel(t), countBy((p) => p.type === t))).join(''));
    el.areaChips.insertAdjacentHTML('beforeend', chip('area', 'all', 'All themes', pubs.length)
      + usedAreas.map((a) => chip('area', a.id, a.title, countBy((p) => (p.areas || []).includes(a.id)), a.hue)).join(''));
    ...
    if (dated.length) {
      ...
      const GLABEL = { journal: 'Journal', conference: 'Conference', other: 'Other' };
      const GVAR = { journal: '--bar-1', conference: '--bar-2', other: '--bar-3' };
      el.chart.innerHTML = `
        <div class="chart">
          <div class="chart__head">
            <div><h2 class="chart__title">Publications per year</h2><p class="chart__hint">Select a year to filter the list below.</p></div>
            <ul class="chart__legend">
              ${present.map((g) => `<li><span class="chart__key" style="--c:var(${GVAR[g]})"></span>${GLABEL[g]}</li>`).join('')}
              <li><span class="chart__key chart__key--mean"></span>Mean ${mean.toFixed(1)} per year</li>
            </ul>
          </div>
          <div class="chart__scroll"><div class="chart__inner" style="--years:${cols.length}">
            <div class="chart__plot">
              ...
              <div class="chart__cols">
                ${cols.map((c, i) => {
                  const detail = ['journal', 'conference', 'other'].filter((g) => c.g[g]).map((g) => `${c.g[g]} ${GLABEL[g].toLowerCase()}`).join(', ');
                  const label = `${c.y}: ${c.n} publication${c.n === 1 ? '' : 's'}`;
    ...
    function renderList() {
      ...
      el.count.textContent = isFiltered() ? `${items.length} of ${plural(pubs.length, 'publication')} match` : plural(pubs.length, 'publication');
      el.clear.hidden = !isFiltered();
      if (!items.length) {
        el.list.innerHTML = `<div class="empty"><p><strong>No publications match these filters.</strong> Try removing a filter or searching for a different word.</p>
          <button type="button" class="btn btn--ghost btn--sm" data-action="clear">Clear filters</button></div>`;
        el.more.hidden = true;
        return;
      }
      ...
      if (state.sort === 'cited') {
        ...
        el.list.innerHTML = [...groups].map(([y, list]) => `
          <section class="year-group" aria-labelledby="year-${y}">
            <h3 class="year-group__label" id="year-${y}">${y || 'Undated'}</h3>
            <ol class="entry-list">${list.map(li).join('')}</ol>
          </section>`).join('');
      }
      const left = items.length - shown.length;
      el.more.hidden = left <= 0;
      if (left > 0) $('button', el.more).textContent = `Show ${Math.min(PAGE_SIZE, left)} more (${left} remaining)`;
    }
    ...
    el.list.addEventListener('click', async (e) => {
      ...
      if (action === 'abstract') {
        ...
        el.status.textContent = ok ? 'BibTeX copied to clipboard.' : 'Could not copy the BibTeX entry.';
  ...
  async function initFeatured(list) {
    let data;
    try { data = await loadAll(); } catch (e) { list.outerHTML = ENE.errorNotice('the featured publications'); return; }
